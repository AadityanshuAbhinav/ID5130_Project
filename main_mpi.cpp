...
int main(int argc, char* argv[])
{
    ...
    // ----------------------------------------------------------------
    // MESH: rank 0 generates, broadcasts raw arrays to all ranks
    // ----------------------------------------------------------------
    ...
    // ----------------------------------------------------------------
    // All ranks build the full mesh struct identically
    // ----------------------------------------------------------------
    ...
    // local A and B — only rows [localStart, localEnd)
    ...
    // ---------------- MATRIX ASSEMBLY (local rows only) ----------------
    ...
    // Graph coloring (node-sharing adjacency) — identical to OMP solver
    ...
    // ----------------------------------------------------------------
    // GRAPH-COLORED GS-SOR: iterate over colors sequentially.
    // Elements in the same color share no node → all ranks update their
    // local slice of each color simultaneously, then Allgatherv syncs.
    // This is equivalent to the OMP update order → same convergence,
    // same iteration count, and stable for any ω ∈ (0,2).
    // ----------------------------------------------------------------
    ...
    for(int iter = 0; iter < maxIter; iter++)
    {
        ...
        {
            // Each rank updates only its local elements from this color.
            // Elements in the same color share no node, so all updates
            // within a color are independent regardless of rank.
            for (int i : group)
            {
            ...
            // Sync after each color so the next color sees fresh values
