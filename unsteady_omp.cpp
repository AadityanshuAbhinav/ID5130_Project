...
double T_analytical(double x, double y, double t, int N)
{
    double T = 0.0;
    ...
    for(int n = 1; n <= N; n += 2)
    {
        ...
        T += (4.0 * T_bottom / (nn * PI)) *
             std::sin(nn * PI * x) *
             std::sinh(nn * PI * (1.0 - y)) /
             std::sinh(nn * PI);
    ...
    for(int m = 1; m <= N; m++)
    {
        for(int n = 1; n <= N; n++)
        {
            ...
            double lambda = PI * PI * (mm*mm + nn*nn);
            double coeff  = 16.0 / (PI*PI*mm*nn);
...
int main(int argc, char* argv[])
{
    ...
    double t_setup_start = omp_get_wtime();
    ...
    E.NeighborFaceArea.resize(ElementNum*6, 0.0);  // 2 components per face
    ...
    #pragma omp parallel for schedule(static) \
        private(cx_i,cy_i,neighbor,DeltaEta,DeltaZhi,nA,nB,idxA,idxB,xA,xB,yA,yB,etaX,etaY)
    for(int i = 0; i < ElementNum; i++)
    {
    ...
    double t_setup_end = omp_get_wtime();
    ...
    std::ofstream file("Usteady.txt");

    double t_solve_start = omp_get_wtime();

    for (int t_step = 0; t_step < nsteps; t_step++)
    {
        t_phys = delta_t * static_cast<double>(t_step);

        // reads T[] (old, read-only this step), writes T_buffer[i] — no race condition
        #pragma omp parallel for schedule(static) \
            firstprivate(Af, EtaCap, ZhiCap, elems) \
            private(T_val, neighbor, idNeighbor, PGcoeff, SGcoeff, DeltaEta, DeltaZhi, nA, nB)
        for (int i = 0; i < ElementNum; i++)
        {
            T_val = 0.0;
            for (int j = 0; j < 3; j++)
        ...
        // parallel region ends here — T_buffer is fully written

        // serial buffer swap
        for (int i = 0; i < ElementNum; i++)
            T[i] = T_buffer[i];

        // write every 10 steps to keep file manageable
        if (t_step % 10 == 0)
        {
            for (int i = 0; i < ElementNum; i++)
    ...
    double t_solve_end = omp_get_wtime();
