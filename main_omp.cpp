...
int main(int argc, char* argv[])
{
    ...
    // ---------------- MATRIX ASSEMBLY (parallel over rows) ----------------
    #pragma omp parallel for schedule(static) \
        firstprivate(Af,EtaCap,ZhiCap,elems) \
        private(neighbor,idNeighbor,PGcoeff,SGcoeff,DeltaEta,DeltaZhi,nA,nB)
    for (int i = 0; i < ElementNum; i++)
    {
    ...
    // ---------------- GS-SOR SOLVER (graph-colored parallel) ----------------
    int maxIter = 10000;
    double tolGS = 1e-10;
    ...
    for (int iter = 0; iter < maxIter; iter++)
    {
        ...
        {
            #pragma omp parallel for schedule(static) reduction(max:maxError)
    ...
    // ---------------- WRITE STEADY RESULT FILE ----------------
    std::ofstream outf("steady_result.txt");
    outf << "x y T T_exact\n";
    for(int i = 0; i < ElementNum; i++)
