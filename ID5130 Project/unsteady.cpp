...
const double delta_t =  (lc * lc) / 20.0;
...
struct Element
{
    ...
    std::vector<double> Volume; // [vol1, vol2, vol3 .....]
...
int main(){
    ...
    for (int i = 0; i < ElementNum; i++)
    {
        ...
        E.Volume[i] = std::pow(S * (S - SideLengthBuffer[0]) * (S - SideLengthBuffer[1]) * (S - SideLengthBuffer[2]), 0.5);
    ...
    for (int t_step = 0; t_step < 10000; t_step++)
    {
        t = delta_t * static_cast<double>(t_step);
        for (int i = 0; i < ElementNum; i++)
        {
            T_val = 0.0;
            //T_exact[i] = T_analytical(E.CentroidCoords[2 * i], E.CentroidCoords[2 * i + 1], t, n);
            for (int j = 0; j < 3; j++)
            {
                neighbor = E.NeighborElementTags[3 * i + j];
                ...
                if(neighbor != 0)
                {
                    ...
                    {
                        T_val -= SGcoeff * T_left;
                    }
                    
                    else if (rightNodes.count(nA))
                    {
                        T_val -= SGcoeff * T_right;
                    }
                    ...
                    else
                    {
                        ...
                        {
                            T_val -= (SGcoeff / elems.size()) * T[elems[element]];
                        }
                    }
            ...
            file << t << " " << E.CentroidCoords[2 * i] << " " << E.CentroidCoords[2 * i + 1] << " " << T_val << std::endl;
