...
double T_analytical(double x, double y, double t, int N)
{
    ...
    for(int n = 1; n <= N; n += 2)
    {
        ...
        T += (4.0 * T_left / (nn * PI)) *
             std::sin(nn * PI * y) * std::sinh(nn * PI * (1.0 - x)) / std::sinh(nn * PI);
        T += (4.0 * T_right / (nn * PI)) *
             std::sin(nn * PI * y) * std::sinh(nn * PI * x) / std::sinh(nn * PI);
        T += (4.0 * T_bottom / (nn * PI)) *
             std::sin(nn * PI * x) * std::sinh(nn * PI * (1.0 - y)) / std::sinh(nn * PI);
        T += (4.0 * T_top / (nn * PI)) *
             std::sin(nn * PI * x) * std::sinh(nn * PI * y) / std::sinh(nn * PI);
    }

    for(int m = 1; m <= N; m++)
        for(int n = 1; n <= N; n++)
        {
            double mm = static_cast<double>(m), nn = static_cast<double>(n);
            double lambda = PI * PI * (mm*mm + nn*nn);
            T += (16.0 / (PI*PI*mm*nn)) *
                 std::sin(mm * PI * x) * std::sin(nn * PI * y) *
                 std::exp(-lambda * t);
        }
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < ElementNum; i++)
    {
        ...
        E.Volume[i] = std::sqrt(S * (S-SideLengthBuffer[0]) *
                                    (S-SideLengthBuffer[1]) *
    ...
    std::ofstream file("Usteady_serial.txt");
        ...
        for (int i = 0; i < ElementNum; i++)
        {
            ...
            for (int j = 0; j < 3; j++)
            {
                ...
                if(neighbor != 0)
                {
                    ...
                    PGcoeff = (Af[0]*Af[0]+Af[1]*Af[1]) /
                              ((Af[0]*ZhiCap[0]+Af[1]*ZhiCap[1]) * DeltaZhi);
                    SGcoeff = -(Af[0]*Af[0]+Af[1]*Af[1]) /
                               ((Af[0]*ZhiCap[0]+Af[1]*ZhiCap[1]) * DeltaEta) *
